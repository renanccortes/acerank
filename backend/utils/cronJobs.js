...
const resetMonthlyDeclines = () => {
  cron.schedule('10 0 1 * *', async () => {
    try {
      ...
      console.log(`✅ Reset mensal concluído! ${result.modifiedCount} jogadores atualizados.`);
      
      // Log detalhado para auditoria
      const now = new Date();
      console.log(`📅 Reset executado em: ${now.toISOString()}`);
      console.log(`📊 Próximo reset: ${new Date(now.getFullYear(), now.getMonth() + 1, 1, 0, 10).toISOString()}`);
...
const cleanupOldData = () => {
  cron.schedule('0 2 * * 0', async () => {
    try {
      ...
      console.log(`🗑️ ${oldChallenges.deletedCount} desafios antigos removidos`);
      ...
      console.log(`🗑️ ${oldMatches.deletedCount} matches pendentes antigos removidos`);
...
const updateRankingsPeriodically = () => {
  ...
  console.log('📊 Cron job de atualização de rankings configurado (a cada 6 horas)');
};
