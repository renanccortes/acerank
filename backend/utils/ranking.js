...
const calculateMatchPoints = (winner, loser) => {
  ...
  const finalLoserPoints = Math.round(
    Math.max(loserPoints, POINTS_CONFIG.MIN_POINTS - loser.points) * multiplier
...
const updateGeneralRanking = async () => {
  const players = await Player.find({ isActive: true }).sort({
    points: -1,
    wins: -1,
    name: 1,
  });
...
const updateGenderRankings = async () => {
  ...
  for (const gender of genders) {
    const players = await Player.find({ isActive: true, gender }).sort({
      points: -1,
      wins: -1,
      name: 1,
    });
...
const updateRegionRankings = async () => {
  ...
  for (const region of regions) {
    const players = await Player.find({ isActive: true, region }).sort({
      points: -1,
      wins: -1,
      name: 1,
    });
...
const updateLevelRankings = async () => {
  ...
  for (const level of levels) {
    const players = await Player.find({ isActive: true, level }).sort({
      points: -1,
      wins: -1,
      name: 1,
    });
...
const canChallenge = async (challenger, challenged) => {
  ...
  if (challenger._id.toString() === challenged._id.toString()) {
    return {
      canChallenge: false,
      reason: 'Não é possível desafiar a si mesmo',
    };
  }
  ...
  if (levelDifference === 0) {
    ...
    if (rankingDifference <= 0) {
      return {
        canChallenge: false,
        reason:
          'Só é possível desafiar jogadores em posições superiores no seu nível',
      };
    }
...
const getPlayerStats = async playerId => {
  ...
  const wins = matches.filter(
    match => match.winner.toString() === playerId
  ).length;
  const losses = matches.filter(
    match => match.loser.toString() === playerId
  ).length;
