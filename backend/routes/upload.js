...
router.post('/match/:matchId', auth, upload.single('resultPhoto'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: 'Nenhum arquivo foi enviado' });
    }
    ...
    const isParticipant = match.player1._id.toString() === req.player.id || 
                         match.player2._id.toString() === req.player.id;
    
    if (!isParticipant) {
      return res.status(403).json({ message: 'Você não tem permissão para adicionar foto a esta partida' });
    }
