  ...
  async (req, res) => {
    try {
      ...
      if (existingChallenge) {
        return res
          .status(400)
          .json({ message: 'Já existe um desafio pendente com este jogador' });
      }
      ...
      try {
        await NotificationService.challengeReceived(
          challenger._id,
          challengedId,
          challenge._id
        );
        ...
        console.error(
          'Erro ao enviar notificação de desafio:',
          notificationError
        );
...
router.put(
  '/:id/respond',
  auth,
  validateSchema(respondChallengeSchema),
  async (req, res) => {
    try {
      ...
      if (challenge.status !== 'pending') {
        return res
          .status(400)
          .json({ message: 'Desafio não está mais pendente' });
      }
      ...
      if (action === 'accept') {
        ...
        if (penaltyResult.penaltyApplied) {
          try {
            ...
            await NotificationService.pointsChanged(
              challenger._id,
              `+10 pts do desafio recusado`
            );
          } catch (notificationError) {
            console.error(
              'Erro ao enviar notificação de pontos:',
              notificationError
            );
          }
        }
      ...
      try {
        if (action === 'accept') {
          await NotificationService.challengeAccepted(
            req.player._id,
            challenge.challenger,
            challenge._id
          );
        } else if (action === 'decline') {
          await NotificationService.challengeDeclined(
            req.player._id,
            challenge.challenger,
            challenge._id
          );
        ...
        console.error(
          'Erro ao enviar notificação de resposta:',
          notificationError
        );
