...
router.get('/challenge/:challengeId', auth, async (req, res) => {
  try {
    ...
    const isInvolved = challenge.challenger.toString() === req.player._id.toString() ||
...
router.post('/challenge/:challengeId/message', auth, async (req, res) => {
  try {
    ...
    const recipientId = req.player._id.toString() === challenge.challenger.toString() 
      ? challenge.challenged 
      : challenge.challenger;
    
    try {
      await NotificationService.newMessage(req.player._id, recipientId, challengeId, chat._id);
...
router.put('/challenge/:challengeId/read', auth, async (req, res) => {
  try {
    ...
    const isParticipant = chat.participants.some(p => p.toString() === req.player._id.toString());
    if (!isParticipant) {
      return res.status(403).json({ message: 'Não autorizado' });
    }
  ...
  try {
    ...
    const chatsWithUnread = chats.map(chat => {
      const unreadCount = chat.messages.filter(msg => 
        msg.sender.toString() !== req.player._id.toString() && !msg.read
      ).length;
