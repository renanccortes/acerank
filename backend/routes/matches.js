...
router.post('/', auth, async (req, res) => {
  try {
    const { challengeId, winnerId, score, sets, matchDate, location, duration, notes } = req.body;
    ...
    const validWinner = winnerId === challenge.challenger._id.toString() ||
    ...
    const winner = winnerId === challenge.challenger._id.toString() ? 
                   challenge.challenger : challenge.challenged;
    const loser = winnerId === challenge.challenger._id.toString() ? 
    ...
    const opponentId = req.player._id.toString() === challenge.challenger._id.toString() 
      ? challenge.challenged._id 
      : challenge.challenger._id;
    
    try {
      await NotificationService.matchResultSubmitted(req.player._id, opponentId, match._id);
    } catch (notificationError) {
      console.error('Erro ao enviar notificação de resultado:', notificationError);
...
router.put('/:id/validate', auth, async (req, res) => {
  try {
    ...
    if (match.loser._id.toString() !== req.player._id.toString()) {
      return res.status(403).json({ message: 'Apenas o perdedor pode validar o resultado' });
    }
    ...
    if (action === 'confirm') {
      ...
      try {
        await NotificationService.matchResultValidated(req.player._id, match.reportedBy, match._id);
      } catch (notificationError) {
        console.error('Erro ao enviar notificação de validação:', notificationError);
      }
      ...
      // Enviar notificação para quem reportou o resultado
      try {
        await NotificationService.matchResultDisputed(req.player._id, match.reportedBy, match._id);
      } catch (notificationError) {
        console.error('Erro ao enviar notificação de contestação:', notificationError);
      }
      
      res.json({ message: 'Resultado contestado. Será analisado pela administração.', match });
    } else {
...
async function applyMatchResults(match) {
  ...
  match.rankingAfter = {
    player1: match.player1.toString() === updatedWinner._id.toString() ? 
      { ranking: updatedWinner.ranking, points: updatedWinner.points } :
      { ranking: updatedLoser.ranking, points: updatedLoser.points },
    player2: match.player2.toString() === updatedWinner._id.toString() ? 
      { ranking: updatedWinner.ranking, points: updatedWinner.points } :
      { ranking: updatedLoser.ranking, points: updatedLoser.points }
  };
}
...
router.get('/my/history', auth, async (req, res) => {
  try {
    ...
    let query = {
      $or: [
        { player1: req.player._id },
        { player2: req.player._id }
      ]
    };
